...
def example_generator(data_path, single_pass, entity_replace):
  ...
  for idx,line in enumerate(reader.readlines()):
    if idx % 5 == 0:
      question_text = line.strip().split('>')[1][1:]
      data_list.append([question_text])
    if idx % 5 == 1:
      logical_text = line.strip().split('>')[1][1:]
      logical_text = '<s> ' + logical_text + ' </s>'
      data_list[-1].append(logical_text)
  if entity_replace:
    for data in data_list:
      logical_token = data[1].split(' ')
      token_ids = []
      combine_tokens = []
      has_entity = False
      for idx, token in enumerate(logical_token):
        if '_' in token:
          combine_token = token.split('_')
          combine_tokens.append(combine_token)
          token_ids.append(idx)
      for idx, c_token in enumerate(combine_tokens):
        combine_string = ' '.join(c_token)
        if combine_string in data[0]:
          logical_token[token_ids[idx]] = c_token
          has_entity = True
      if has_entity:
        logical_token = flatten(logical_token)
        data[1] = ' '.join(logical_token)
  if single_pass:
    for data in data_list:
      yield (data[0], data[1])
